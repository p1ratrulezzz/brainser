...
build:
	...
	CC=x86_64-w64-mingw32-gcc CGO_ENABLED=1 GOARCH=amd64 GOOS=windows go build -o "${OUTDIR}/${BINARY_NAME}-win-x64.exe" "${SRCPATH}"
	CGO_ENABLED=1 GOARCH=arm64 GOOS=linux go build -o "${OUTDIR}/${BINARY_NAME}-linux-arm64" "${SRCPATH}"
	CC=x86_64-w64-mingw32-gcc CGO_ENABLED=1 GOARCH=arm64 GOOS=windows go build -o "${OUTDIR}/${BINARY_NAME}-win-arm64.exe" "${SRCPATH}"
